...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for position, is_relevant in enumerate(relevant_flags, start=1):
            if is_relevant:
                running_relevant += 1
                precision_at_k = running_relevant / position
...
class LLMJudge:
    ...
    @staticmethod
    def _parse_scores(
        response: str, criteria: dict[str, Any]
    ) -> tuple[dict[str, float], str]:
        """Parse judge output into {criterion: score} plus the raw explanation.

        Understands plain JSON like {"accuracy": 0.8} and wrapped JSON like
        {"scores": {...}, "reasoning": "..."}. Falls back to 0.5 per criterion.
        """
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial judge. Score the following assistant answer "
            "toward the question, using a scale of 0.0 to 1.0 per criterion.\n\n"
            f"Question:\n{question}\n\n"
            f"Assistant answer:\n{answer}\n\n"
            "Rubric criteria:\n"
            f"{rubric_text}\n\n"
            "Return JSON only, e.g. "
            '{"criterion_name": 0.8, "reasoning": "one line explanation"}.'
        )
        response = self.judge_llm_fn(prompt)
        scores, reasoning = self._parse_scores(response, rubric)
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def _mean_scores(entry: dict[str, Any]) -> float:
            values = [v for v in entry.get("scores", {}).values() if isinstance(v, (int, float))]
            return statistics.mean(values) if values else 0.0

        means = [_mean_scores(entry) for entry in scores_batch]
        overall = statistics.mean(means)
        positional = False
        if len(means) >= 2:
            rest = statistics.mean(means[1:])
            positional = means[0] > rest + 0.1
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            results.append(replace(result, qa_pair=pair))
        return results
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": (
                statistics.mean(recall_scores) if recall_scores else None
            ),
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        new_avg_faithfulness = statistics.mean(
            [result.faithfulness for result in new_results]
        )
        new_avg_relevance = statistics.mean(
            [result.relevance for result in new_results]
        )
        new_avg_completeness = statistics.mean(
            [result.completeness for result in new_results]
        )
        baseline_avg_faithfulness = statistics.mean(
            [result.faithfulness for result in baseline_results]
        )
        baseline_avg_relevance = statistics.mean(
            [result.relevance for result in baseline_results]
        )
        baseline_avg_completeness = statistics.mean(
            [result.completeness for result in baseline_results]
        )
        ...
        regressions = [
            name
            for name, (new_value, baseline_value) in comparisons.items()
            if new_value < baseline_value - 0.05
        ]
        return {
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest = min(metrics.values())
        tied = [name for name, score in metrics.items() if score == lowest]
        if len(tied) > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            fix = (
                suggestions[index - 1]
                if index - 1 < len(suggestions) and suggestions[index - 1]
                else "Review and refine pipeline"
            )
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type: dict[str, str] = {
            "hallucination": (
                "Implement a hallucination checker to filter claims not grounded "
                "in the retrieved context"
            ),
            "irrelevant": (
                "Improve prompt clarity and intent extraction so answers address "
                "the actual question"
            ),
            "incomplete": (
                "Increase chunk size in the RAG pipeline to reduce context "
                "fragmentation and missing details"
            ),
            "off_topic": (
                "Add intent detection and routing to keep answers on the requested "
                "topic"
            ),
            "refusal": (
                "Loosen over-strict guardrails so in-scope customer-support "
                "requests are not refused"
            ),
        }
        categories = self.categorize_failures(failures)
        suggestions = [
            message
            for failure_type, message in suggestions_by_type.items()
            if failure_type in categories
        ]
        generic = [
            "Add few-shot examples showing complete customer-support answers",
            "Augment the golden dataset with more cases for the recurring failure",
            "Review the retrieval query formulation and source-document coverage",
        ]
        for index, suggestion in enumerate(generic):
            if len(suggestions) >= 3:
                break
            suggestions.append(suggestion)
        return suggestions[: max(3, len(categories))] if suggestions else generic[:3]
